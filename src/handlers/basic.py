...
async def show_all_dates(message: types.Message, dates: list):
    if message.from_user.id == int(os.getenv('SUDO_ID')) or await db.is_admin(message.from_user.id):
        await message.answer('Вы перешли в раздел доступных дат', reply_markup=cancel_buttons)
        await message.answer("Доступные даты:", reply_markup=get_dates_ikb(dates))
...
async def cmd_help(message: types.Message):
    ...
    instagram = ("· Instagram: @verasok_lashes_brows\n"
                 "· Telegram: @verasok83")
    text = f'{about}\n Мои соцсети:\n <b>{instagram}</b>'
    await message.answer(text, parse_mode='HTML', reply_markup=cancel_buttons)
